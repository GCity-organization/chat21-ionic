...
export class TiledeskService {
  ...
  public closeSupportGroup(
    ...
    const url =
      this.apiUrl + projectid + "/requests/" + supportgroupid + "/close";
  ...
  public getProjectIdByConvRecipient(token: string, conversationWith: string) {
    const lookupUrl =
      this.apiUrl + "requests_util/lookup/id_project/" + conversationWith;

    this.logger.log(
      "[TILEDESK-SERVICE] GET PROJECTID BY CONV RECIPIENT - URL ",
      lookupUrl
    );
    ...
    return this.http.get(lookupUrl, httpOptions).pipe(
      map((res: any) => {
        this.logger.log(
          "[TILEDESK-SERVICE] GET PROJECTID BY CONV RECIPIENT - RES ",
          res
        );
        return res;
      })
  ...
  public getAllLeadsActiveWithLimit(
    project_id: string,
    token: string,
    limit: number
  ) {
    const url =
      ...
      "&with_fullname=true";
    this.logger.log(
      "[TILEDESK-SERVICE] - GET ALL ACTIVE LEADS (LIMIT 10000) -  URL",
      url
    );
    ...
    return this.http.get(url, httpOptions).pipe(
      map((res: any) => {
        this.logger.log(
          "[TILEDESK-SERVICE] - GET ALL ACTIVE LEADS (LIMIT 10000) ",
          res
        );
        return res;
      })
  ...
  public createNewProjectUserToGetNewLeadID(project_id: string, token: string) {
    const url = this.apiUrl + project_id + "/project_users/";
    this.logger.log(
      "[TILEDESK-SERVICE] - CREATE NEW PROJECT USER TO GET NEW LEAD ID url ",
      url
    );
    const httpOptions = {
      headers: new HttpHeaders({
  ...
  public getAllBotByProjectId(project_id: string, token: string) {
    ...
    this.logger.log(
      "[TILEDESK-SERVICE] - GET ALL BOTS BY PROJECT ID - URL",
      url
    );
    ...
    return this.http.get(url, httpOptions).pipe(
      map((res: any) => {
        this.logger.log(
          "[TILEDESK-SERVICE] - GET ALL BOTS BY PROJECT ID - RES ",
          res
        );
        return res;
      })
  ...
  public getDeptsByProjectId(project_id: string, token: string) {
    ...
    return this.http.get(url, httpOptions).pipe(
      map((res: any) => {
        this.logger.log(
          "[TILEDESK-SERVICE] - GET DEPTS (ALL STATUS) - RES ",
          res
        );
        return res;
      })
  ...
  public createInternalRequest(
    ...
    const url =
      this.apiUrl + project_id + "/requests/" + request_id + "/messages";
  ...
  public sendEmail(
    ...
    projectid: string,
    request_id: string,
    form: { subject: string; text: string }
  ) {
    const httpOptions = {
      headers: new HttpHeaders({
        Accept: "application/json",
    ...
    const url =
      this.apiUrl + projectid + "/requests/" + request_id + "/email/send";
  ...
  // @ Send Email
  // -----------------------------------------------------------------------------------------
  public sendWhatsappMessage(token: string, projectid: string, form: any) {
    const httpOptions = {
      headers: new HttpHeaders({
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    const url = this.apiUrl + "modules/whatsapp/tiledesk/broadcast";
    this.logger.log("[TILEDESK-SERVICE] - createWhatsappMessage URL ", url);
    return this.http.post(url, body, httpOptions).pipe(
      map((res: any) => {
        this.logger.log("[TILEDESK-SERVICE] - createWhatsappMessage - RES ", res);
        return res;
      })
    );
  }
